...
from vela.utils.color import get_dynamic_colours
from vela.utils.logging import log_exception
from vela.utils.paths import (
    c_state_dir,
    config_dir,
    data_dir,
    templates_dir,
...
def apply_discord(scss: str) -> None:
    ...
    for client in "Equicord", "Vencord", "BetterDiscord", "equibop", "vesktop", "legcord":
        write_file(config_dir / client / "themes/vela.theme.css", conf)


@log_exception
def apply_spicetify(colours: dict[str, str], mode: str) -> None:
    template = gen_replace(colours, templates_dir / f"spicetify-{mode}.ini")
    write_file(config_dir / "spicetify/Themes/vela/color.ini", template)
...
def apply_btop(colours: dict[str, str]) -> None:
    ...
    write_file(config_dir / "btop/themes/vela.theme", template)
...
def apply_qt(colours: dict[str, str], mode: str) -> None:
    ...
    write_file(config_dir / "qt5ct/colors/vela.colors", template)
    write_file(config_dir / "qt6ct/colors/vela.colors", template)
...
def apply_warp(colours: dict[str, str], mode: str) -> None:
    ...
    write_file(data_dir / "warp-terminal/themes/vela.yaml", template)
